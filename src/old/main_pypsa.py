...
from OR_tools.src.old.pypsa_optimizer import PyPSAOptimizer
import sys
import os
import pandas as pd
...
def main():
    ...
    try:
        ...
        if results is not None:
            ...
            output_path = "results/pypsa_results.xlsx"
            results_rounded.to_excel(output_path)
            print(f"\nSUCCESS! PyPSA Results saved to: {output_path}")
            print(results[["chp_heat_out", "heat_demand"]].head(10))
            total_gas = results["chp_gas_in"].sum()
            print(f"Total Gas Consumed (PyPSA): {total_gas:,.2f} MWh")
        else:
            print("PyPSA optimization finished but returned no results.")
    except Exception as e:
        print(f"PyPSA Optimization Crash: {e}")
        import traceback
        traceback.print_exc()
